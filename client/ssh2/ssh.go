...
func (s *SSH) CombinedOutput(cmd string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("exec cmd %q error: %w", cmd, err)
	}
	if exit != 0 {
		return nil, fmt.Errorf("exec cmd %q error: exit code %d: stderr %s", cmd, exit, stderr)
	}
...
func (s *SSH) writeFile(src io.Reader, dst string) error {
	...
	if s.Sudo {
		_, err = s.CombinedOutput(fmt.Sprintf("chown root:root %s", realDst))
		if err != nil {
			return fmt.Errorf("failed to chown %s on %s, err: %v", realDst, s.Host, err)
		}
	}
...
func (s *SSH) newClient() (*ssh.Client, func(), error) {
	...
	client, err := s.dialer.Dial("tcp", s.addr(), config, s)
	if err != nil {
		return nil, nil, err
	}
...
type sshDialer interface {
	Dial(network, addr string, config *ssh.ClientConfig, sshConfig *SSH) (*ssh.Client, error)
}
...
func (d *realSSHDialer) Dial(network, addr string, config *ssh.ClientConfig, sshConfig *SSH) (*ssh.Client, error) {
	var conn net.Conn
	proxyConfig := sshConfig.Proxy
	if proxyConfig != nil {
		switch proxyConfig.Type {
		case "SOCKS5":
			dialer, err := proxy.SOCKS5("tcp", proxyConfig.Address, proxyConfig.Auth, &net.Dialer{
		...
		default:
			return nil, fmt.Errorf("unknown proxy type: %s", proxyConfig.Type)
		}
	} else {
		var err error
		log.Debugf("DialTimeout %s://%s %v", network, addr, config.Timeout.Seconds())
		dialer := &net.Dialer{
			Timeout: config.Timeout,
		}
		if conn, err = dialer.Dial(network, addr); err != nil {
			log.Debugf("Dial %s://%s %v error: %v", network, addr, config.Timeout.Seconds(), err)
			return nil, err
		}
		/*
			...
			if conn, err = dialer.DialContext(ctx, network, addr); err != nil {
				log.Warnf("DialTimeout %s://%s %v error: %v", network, addr, config.Timeout.Seconds(), err)
				return nil, err
			}
			log.Warnf("DialTimeout %s://%s %v finish", network, addr, config.Timeout.Seconds())
		*/
	}
	conn.SetDeadline(time.Now().Add(sshConfig.DialTimeOut))

	c, chans, reqs, err := ssh.NewClientConn(conn, addr, config)
	if err != nil {
		return nil, err
	}
	conn.SetDeadline(time.Now().Add(sshConfig.ReadTimeOut))
...
func (d *timeoutDialer) Dial(network, addr string, config *ssh.ClientConfig, sshConfig *SSH) (*ssh.Client, error) {
	config.Timeout = d.timeout
	return d.dialer.Dial(network, addr, config, sshConfig)
}
